      ...
      {isLoading ? (
        ...
        <div>
          {orders.map((order) => (
            <Card
              key={order._id}
              style={{ marginBottom: "24px", border: "solid 1px #db4444" }}>
              <Card.Body>
                <b>Order ID</b>
                ...
                <b>Nomor Resi</b>
                <Card.Text>{order.resi ? order.resi : "Belum ada resi"}</Card.Text>
                <b>Alamat</b>
                <Card.Text>{order.shippingAddress}</Card.Text>
                <b>Status</b>
                <Card.Text>{order.status}</Card.Text>
                <b>Tanggal Pesanan</b>
                <Card.Text>{new Date(order.createdAt).toLocaleString("id-ID")}</Card.Text>
                {order.status === "Belum Dibayar" && (
                  <Card.Text>
                    Hubungi <a href={`https://wa.me/6285745820511?text=Halo%20saya%20ingin%20membayar%20pesanan%20dengan%20id%20${order._id}`}>085745820511</a> untuk melakukan pembayaran, admin akan mengonfirmasi pembayaran agar barang
                    segera di proses
                  </Card.Text>
                )}
                {order.status === "Diproses" && (
                  <Card.Text>
                    Barang sedang diproses, nomor resi akan tertera disini saat barang sudah dikirim. jika ingin melakukan pembatalan segera konfirmasi ke{" "}
                    <a href={`https://wa.me/6285745820511?text=Halo%20saya%20ingin%20membatalkan%20pesanan%20dengan%20id%20${order._id}`}> 085745820511 </a> (admin)
                  </Card.Text>
                )}
              </Card.Body>
              {(order.status === "Dikirim" || (order.status !== "Dibatalkan" && order.status !== "Selesai")) && (
                <Card.Footer>
                  {order.status === "Dikirim" && (
                    <button
                      className="button-order-green"
                      onClick={() => handleSelesaiOrder(order._id)}>
                      Selesai
                    </button>
                  )}
